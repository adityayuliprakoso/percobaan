	...
	function __construct()
  {
	...
	function action()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			redirect(base_url('member/login?page=login&status=500&msg='.bin2hex($msg)));
		}
		else
		{
			$username = $_POST['login_username'];
			$password = $_POST['login_password'];
			$login_proc = $auth->login_member($username, $password);
			$msg = $auth->get_message($login_proc['msg']);
			redirect(base_url('member/'.$login_proc['redirect'].'?page='.$login_proc['page'].'&status='.$login_proc['status'].'&msg='.bin2hex($msg)));
		}
	}
	...
	function logout()
	{
		...
		$msg = $auth->get_message($logout_proc['msg']);
			redirect(base_url('member/login?page=login&status='.$logout_proc['status'].'&msg='.bin2hex($msg)));
	}
}
